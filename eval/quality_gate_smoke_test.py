...
def main() -> None:
    baseline = payload({"recall_at_5": 1.0, "mrr_at_10": 0.9, "negative_no_answer_rate": 0.5, "citation_coverage": 1.0})
    acceptable = payload({"recall_at_5": 0.96, "mrr_at_10": 0.88, "negative_no_answer_rate": 0.48, "citation_coverage": 0.96})
    assert not quality_gate.compare_metrics(baseline, acceptable, quality_gate.DEFAULT_MIN_DELTAS)

    regressed = payload({"recall_at_5": 0.9, "mrr_at_10": 0.8, "negative_no_answer_rate": 0.3, "citation_coverage": 0.8})
